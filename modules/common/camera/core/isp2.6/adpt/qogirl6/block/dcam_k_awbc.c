...
#if 0 //SPCSS01149389
    pr_err("qsl  [%d %d  %d %d]  thrd [%d %d %d] offset[%d %d  %d %d] \n",
    p->gain.b, p->gain.r, p->gain.gb, p->gain.gr, p->thrd.b, p->thrd.r, p->thrd.gr, p->gain_offset.b,
    p->gain_offset.r, p->gain_offset.gb, p->gain_offset.gr);        
    pr_err("qsl  0x%lx 0x%lx  0x%lx  0x%lx  0x%lx   0x%lx  0x%lx\n",
           DCAM_REG_RD(idx, ISP_AWBC_GAIN0),
           DCAM_REG_RD(idx, ISP_AWBC_GAIN1),
           DCAM_REG_RD(idx, ISP_AWBC_THRD),
           DCAM_REG_RD(idx, ISP_AWBC_THRD1),
           DCAM_REG_RD(idx, ISP_AWBC_OFFSET0),
           DCAM_REG_RD(idx, ISP_AWBC_OFFSET1),
           DCAM_REG_RD(idx, ISP_AWBC_STATUS));
#endif
	return ret;
}
...
#if 0	 //SPCSS01149389
    pr_err("qsl  p->b %d p->r %d  p->gb %d p->gr %d\n",
                 p->b, p->r, p->gb, p->gr);        
    pr_err("qsl  0x%lx 0x%lx  0x%lx  0x%lx  0x%lx   0x%lx  0x%lx\n",
                        DCAM_REG_RD(idx, ISP_AWBC_GAIN0),
                        DCAM_REG_RD(idx, ISP_AWBC_GAIN1),
