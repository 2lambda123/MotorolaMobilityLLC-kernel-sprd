...
extern int fts_get_tp_gesture_status(int);
extern void nvt_esd_check_recovery(void);
...
int g_trigger_disp_esd_recovery = 0;

void set_cs_to_gpio(void)
{
  int ret;
  void __iomem* addr;

  addr = ioremap(0x647101E0, 128);

  if (addr) {
	ret = readl_relaxed(addr);
	ret =  ret | 0x00000030;
	writel_relaxed(ret, addr);
	iounmap(addr);
  }

}

void set_gpio_to_cs(void)
{
  int ret;
  void __iomem* addr;
...
void set_mosi_to_gpio(void)
{
  int ret;
  void __iomem* addr;

  addr = ioremap(0x647101EC, 128);
...
int set_cs_gpio(int value)
{
   int ret;
   ret = gpio_request(158, "cs_gpio");
   ret = gpio_direction_output(158, value);
   return 0;
}

int set_mosi_gpio(int value)
{
   int ret;
   ret = gpio_request(161, "mosi_gpio");
   ret = gpio_direction_output(161, value);
   return 0;
}
...
static int sprd_panel_unprepare(struct drm_panel *p)
{
	...
	if( strncmp(panel->lcd_name,"lcd_ft8726_csot_mipi_fhd",sizeof("lcd_ft8726_csot_mipi_fhd")) ==0 ) {
		mdelay(1);
		fts_gesture = fts_get_tp_gesture_status(panel->enabled);
	}

	if( strncmp(panel->lcd_name,"lcd_nt36672c_tm_mipi_fhd",sizeof("lcd_nt36672c_tm_mipi_fhd")) ==0 || strncmp(panel->lcd_name,"lcd_nt36672c2_tm_mipi_fhd",sizeof("lcd_nt36672c2_tm_mipi_fhd")) ==0 ) {
		nvt_gesture = nvt_get_tp_gesture_status(panel->enabled);
	}

	if( strncmp(panel->lcd_name,"lcd_ili7807s_txd_mipi_fhd",sizeof("lcd_ili7807s_txd_mipi_fhd")) ==0 ) {
		ili_gesture = ili_get_tp_gesture_status(panel->enabled);
	}
	DRM_INFO("fts_gesture = %d, nvt_gesture = %d ,ili_gesture = %d\n ", fts_gesture, nvt_gesture,ili_gesture);
	...
	if((fts_gesture || nvt_gesture || ili_gesture) && !panel_remove){
		regulator_disable(panel->supply);

		if(g_trigger_disp_esd_recovery == 0){
			return 0;
		}
	}
	...
	if (panel->info.vdd_gpio) {
		gpiod_direction_output(panel->info.vdd_gpio, 0);
		mdelay(30);
	}
...
static int sprd_panel_prepare(struct drm_panel *p)
{
	...
	unsigned char data1 = 0x13;
	unsigned char data2 = 0x13;
	...
	if (panel->info.vdd_gpio) {
		gpiod_direction_output(panel->info.vdd_gpio, 1);
		mdelay(5);
	}
	...
	if(strncmp(panel->lcd_name,"lcd_nt36672c_tm_mipi_fhd",sizeof("lcd_nt36672c_tm_mipi_fhd")) == 0 || strncmp(panel->lcd_name,"lcd_nt36672c2_tm_mipi_fhd",sizeof("lcd_nt36672c2_tm_mipi_fhd")) == 0) {
		...
		if(g_trigger_disp_esd_recovery) {
			...
			sprd_panel_send_cmds(panel->slave,
					panel->info.cmds[CMD_CODE_MIPI_IN],
					panel->info.cmds_len[CMD_CODE_MIPI_IN]);
			nvt_esd_vdd_tp_recovery();
			//LCD MIPI 4BC0 = 0x1F
			sprd_panel_send_cmds(panel->slave,
					panel->info.cmds[CMD_CODE_MIPI_OUT],
					panel->info.cmds_len[CMD_CODE_MIPI_OUT]);
		}
	}
	g_trigger_disp_esd_recovery = 0;
...
static int sprd_panel_esd_check(struct sprd_panel *panel)
{
	 ...
	 */
	if(strncmp(panel->lcd_name,"lcd_nt36672c_tm_mipi_fhd",sizeof("lcd_nt36672c_tm_mipi_fhd")) == 0 || strncmp(panel->lcd_name,"lcd_nt36672c2_tm_mipi_fhd",sizeof("lcd_nt36672c2_tm_mipi_fhd")) == 0) {
		mipi_dsi_dcs_read(panel->slave, 0xDE,
				&read_val2, 1);
		mipi_dsi_set_maximum_return_packet_size(panel->slave, 2);
		mipi_dsi_dcs_read(panel->slave, 0xAB,
				&read_val1, 2);

		if ((read_val != info->esd_check_val) || (read_val1 != 0x0000) || ((read_val2 != 0x00) && (read_val2 != 0x01))) {
			DRM_ERROR("esd check failed, read value = 0x%02x  AB = 0x%04x  DE = 0x%02x \n",
					read_val,read_val1,read_val2);
			return -EINVAL;
		}
	}
	else{
...
static void sprd_panel_esd_work_func(struct work_struct *work)
{
	...
	if (ret && panel->base.connector && panel->base.connector->encoder) {
		...
		if( strncmp(panel->lcd_name,"lcd_nt36672c_tm_mipi_fhd",sizeof("lcd_nt36672c_tm_mipi_fhd")) ==0 || strncmp(panel->lcd_name,"lcd_nt36672c2_tm_mipi_fhd",sizeof("lcd_nt36672c2_tm_mipi_fhd")) ==0 ) {
			printk(KERN_ERR"tp  nvt esd recovery \n");
			nvt_esd_check_recovery();
		}
...
static int sprd_panel_gpio_request(struct device *dev,
			struct sprd_panel *panel)
{
	panel->info.vdd_gpio = devm_gpiod_get_optional(dev,
					"vdd", GPIOD_ASIS);
	if (IS_ERR_OR_NULL(panel->info.vdd_gpio))
		DRM_WARN("can't get panel vdd gpio: %ld\n",
				PTR_ERR(panel->info.vdd_gpio));
...
static int sprd_oled_set_brightness(struct backlight_device *bdev)
{
	...
	if (oled->cmds_total == 1) {
		if (oled->cmds[0]->wc_l == 3) {
			if(strncmp(panel->lcd_name,"lcd_ft8726_csot_mipi_fhd",sizeof("lcd_ft8726_csot_mipi_fhd")) == 0) {
					oled->cmds[0]->payload[1] = (set_Brightness >> 3) & 0xFF; /*11bit 2047*/
					oled->cmds[0]->payload[2] = (set_Brightness << 1) & 0x0E;
					//DRM_ERROR("The panel name is ft8726_csot\n");
				} else if(strncmp(panel->lcd_name,"lcd_ili7807s_txd_mipi_fhd",sizeof("lcd_ili7807s_txd_mipi_fhd")) == 0) {
					oled->cmds[0]->payload[1] = (set_Brightness >> 8) & 0x07; /*11bit 2047*/
					oled->cmds[0]->payload[2] = set_Brightness & 0xFF;
					//DRM_ERROR("The panel name is ili7807s_txd\n");
				} else {
					oled->cmds[0]->payload[1] = (set_Brightness >> 8) & 0x07; /*11bit 2047*/
					oled->cmds[0]->payload[2] = set_Brightness & 0xFF;
					//DRM_ERROR("The panel name is nt36672c_tm\n");
				}
	...
	struct sprd_panel *panel)
{
	...
	p = of_get_property(lcd_node, "sprd,mipi-in-command", &bytes);
	if (p) {
		info->cmds[CMD_CODE_MIPI_IN] = p;
		info->cmds_len[CMD_CODE_MIPI_IN] = bytes;
	} else
		DRM_ERROR("can't find sprd,mipi-in-command property\n");

	p = of_get_property(lcd_node, "sprd,mipi-out-command", &bytes);
	if (p) {
		info->cmds[CMD_CODE_MIPI_OUT] = p;
		info->cmds_len[CMD_CODE_MIPI_OUT] = bytes;
	} else
		DRM_ERROR("can't find sprd,mipi-out-command property\n");
