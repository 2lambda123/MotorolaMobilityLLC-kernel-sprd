...
#define pr_fmt(fmt) pr_sprd_fmt("HDST518")""fmt
...
#define LG "%s STS0(184) %x, INT5(214.IEV) %x, INT6(218.IE) %x, INT7(21C.RIS) %x, INT8(220.MIS) %x, INT11(22C.STS1) %x, INT34(280.INTC.EN) %x, INT36(288) %x"
...
#define T32 sprd_read_reg_value(ANA_INT34)
#define T34 sprd_read_reg_value(ANA_INT36)
...
{
	if (enable)
		headset_reg_set_bits(ANA_INT34, ANA_INT_EN);
	else
		headset_reg_clr_bits(ANA_INT34, ANA_INT_EN);
}

void sprd_codec_intc_enable(bool enable, u32 irq_bit)
{
	if (enable)
		headset_reg_set_bits(ANA_INT34, irq_bit);
	else
		headset_reg_clr_bits(ANA_INT34, irq_bit);
}
...
static void sprd_headset_reset(struct sprd_headset *hdst)
{
	...
	pr_err(LG, FC, S0, T5, T6, T7, T8, T11, T32, T34);
	sprd_intc_force_clear(true, ANA_INT35_CODEC_INTC_CLR(0x7fff));
	sprd_ldetl_filter_enable(false);
	sprd_headset_ldetl_ref_sel(LDETL_REF_SEL_100mV);
	...
	sprd_headset_clear_all_eic();
	sprd_set_all_eic_trig_level(true);
	sprd_intc_force_clear(false, ANA_INT35_CODEC_INTC_CLR(0x7fff));
...
static int sprd_get_adc_cal_from_efuse(struct platform_device *pdev)
{
	...
	if (delta[3] & 0x80)
		adc_cal_headset.E2 = (delta[3] - 0x80 + 1) * 4 + 1300;
	else
