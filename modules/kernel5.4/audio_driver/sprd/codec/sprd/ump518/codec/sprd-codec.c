...
#define pr_fmt(fmt) pr_sprd_fmt("UMP518")""fmt
...
#define SPRD_CODEC_EFUSE_FGU_4P2_MASK GENMASK(15, 7)
...
enum {
	...
	SPRD_CODEC_SDA_EN_ORDER = 103,
	SPRD_CODEC_DA_EN_ORDER = 104,
	SPRD_CODEC_DC_OS_SWITCH_ORDER = 105,
	SPRD_CODEC_DC_OS_ORDER = 106,
//	SPRD_CODEC_AO_EN_ORDER = 107,
//	SPRD_CODEC_RCV_DEPOP_ORDER = 108,
	SPRD_CODEC_HP_DALR_ORDER = 107,
	SPRD_CODEC_AO_EN_ORDER = 108,
	SPRD_CODEC_RCV_DEPOP_ORDER = 109,
...
static const struct snd_kcontrol_new ao_gain_controls[] = {
	SOC_SINGLE_EXT_TLV("AO Playback Volume", SOC_REG(ANA_RSV8), AO_G_S,
...
static void sprd_codec_pa_boost(struct snd_soc_component *codec, int pa_d_en)
{
	...
	if (pa_d_en) {
		...
		value = PABST_V_CAL(sprd_codec->aud_pabst_vcal);
		       ...
		       struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
	case SND_SOC_DAPM_POST_PMU:
		snd_soc_component_update_bits(codec, SOC_REG(ANA_RSV8), AO_EN, AO_EN);
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		/* mdelay(1); */
		snd_soc_component_update_bits(codec, SOC_REG(ANA_RSV8), AO_EN, 0);
		break;
	default:
		pr_err("%s invalid event error 0x%x\n", __func__, event);
		break;
			   ...
			   struct snd_kcontrol *kcontrol, int event)
{
	...
	sp_asoc_pr_info("'%s' %s\n", w->name, get_event_name(event));
	if (on)
		val = sprd_codec->dac_switch & SDAAOL_AO;
	snd_soc_component_update_bits(codec, SOC_REG(ANA_RSV8), SDAAOL_AO, val);
			   ...
			   struct snd_kcontrol *kcontrol, int event)
{
	...
	sp_asoc_pr_info("'%s' %s\n", w->name, get_event_name(event));
	if (on)
		val = sprd_codec->dac_switch & SDAAOR_AO;
	snd_soc_component_update_bits(codec, SOC_REG(ANA_RSV8), SDAAOR_AO, val);
...
static void sprd_codec_set_fgu_v_unit(struct snd_soc_component *codec)
{
	...
	fgu_4p2 = (sprd_codec->fgu_4p2_efuse & SPRD_CODEC_EFUSE_FGU_4P2_MASK) >> 7;
			 ...
			 struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		/* select capless mode*/
		snd_soc_component_update_bits(codec, SOC_REG(ANA_RSV6), ADPGAL_MODE_SEL, 0);
		udelay(4);
		snd_soc_component_update_bits(codec, SOC_REG(ANA_RSV6), ADPGAL_REG_EN, 0);
		udelay(4);
		snd_soc_component_update_bits(codec, SOC_REG(ANA_RSV6),
					ADPGAL_CCAP_SEL(0x1f), ADPGAL_CCAP_SEL(0x1f));
		udelay(4);
		break;
	case SND_SOC_DAPM_POST_PMU:
		snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC0), ADL_RST, 0);
			 ...
			 struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		/* select capless mode*/
		snd_soc_component_update_bits(codec, SOC_REG(ANA_RSV7), ADPGAR_MODE_SEL, 0);
		udelay(4);
		snd_soc_component_update_bits(codec, SOC_REG(ANA_RSV7), ADPGAR_REG_EN, 0);
		udelay(4);
		snd_soc_component_update_bits(codec, SOC_REG(ANA_RSV7),
					ADPGAR_CCAP_SEL(0x1f), ADPGAR_CCAP_SEL(0x1f));
		udelay(4);
		break;
	case SND_SOC_DAPM_POST_PMU:
		snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC0), ADR_RST, 0);
				...
				struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		neg_cp = (sprd_codec->neg_cp_efuse >> 8) & 0xff;
...
/* DA route */
	...
	SND_SOC_DAPM_PGA_S("DRV CPDLY PD", 100,
			   SOC_REG(ANA_CDC12), DRV_CPDLY_PD_S, 0, NULL, 0),
	SND_SOC_DAPM_PGA_S("AMPG BUF BYPASS", -1,
			   SOC_REG(ANA_RSV9), AMPG_BUF_BYPASS_S, 0, NULL, 0),
...
/* LineOut */
	...
	SND_SOC_DAPM_PGA_E("AO CUR", SOC_REG(ANA_RSV8), AO_CUR_S, 0,
			   NULL, 0, codec_widget_event,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_PGA_E("AO PA", SND_SOC_NOPM, 0, 0,
			   NULL, 0,
			   codec_widget_event,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
...
/* DA route */
	...
	{"DRV CPDLY PD", NULL, "DAC Virt"},
	{"AMPG BUF BYPASS", NULL, "DRV CPDLY PD"},
	{"DAC Gain", NULL, "AMPG BUF BYPASS"},
...
/* LineOut */
	{"DACS Switch", NULL, "DAC Gain"},
	{"DRV SOFT T", NULL, "DACS Switch"},
	{"AO Virt", NULL, "DRV SOFT T"},

	{"AO Mixer", "AOR Switch", "AO Virt"},
	{"DAS DC Offset", NULL, "AO Mixer"},
	{"DA AOR", "Switch", "DAS DC Offset"},/* AOR not in demux */
	{"SDAAOR AO", NULL, "DA AOR"},
	{"AO Virt2", NULL, "SDAAOR AO"},

	{"AO Mixer", "AOL Switch", "AO Virt"},
	{"DAS DC Offset", NULL, "AO Mixer"},
	{"SDAAOL AO", NULL, "DAS DC Offset"},
				      ...
				      struct snd_ctl_elem_value *ucontrol)
{
	...
	sp_asoc_pr_info("%s, chip_id = %d, ver_id=%d, AUDIO_CODEC_2730=%d\n",
		__func__, chip_id, ver_id, AUDIO_CODEC_518);

	ucontrol->value.integer.value[0] = AUDIO_CODEC_518;
	return 0;
}
...
static int sprd_codec_probe(struct platform_device *pdev)
{
	...
	pr_info("sprd_codec->neg_cp_efuse = 0x%x, sprd_codec->fgu_4p2_efuse = 0x%x",
...
MODULE_LICENSE("GPL");
MODULE_ALIAS("codec:sprd-codec-518");