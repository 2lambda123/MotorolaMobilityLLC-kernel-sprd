					 ...
					 struct sprd_asoc_ext_hook *ext_hook)
{
	...
	const char *prop_sw_gpio = "sprd,spk-ext-sw-gpio";
	...
	for (i = 0; i < spk_cnt; i++) {
		...
		if (ret == 0) {
			...
			gpio_set_value(hook_spk_priv.gpio[ext_ctrl_type], EN_LEVEL);
			dev_info(dev, "Gpio [%s] set low!\n", prop_pa_gpio);	
		} else if (ret < 0) {
	...
	ret = of_get_named_gpio_flags(np, prop_sw_gpio, 0, NULL);
	if (ret < 0) {
		dev_err(dev, "Get gpio %s failed:%d!\n", prop_sw_gpio, ret);
		return 0;
	}
	hook_spk_priv.sw_gpio = ret;
	pr_info("%s gpio %d", prop_sw_gpio, ret);
	gpio_flag = GPIOF_DIR_OUT;
	gpio_flag |= EN_LEVEL ? GPIOF_INIT_HIGH : GPIOF_INIT_LOW;
	ret = gpio_request_one(hook_spk_priv.sw_gpio, gpio_flag, NULL);
	dev_info(dev, "Gpio request[%s] ret:%d!\n", prop_sw_gpio, ret);
	if (ret == 0) {
		gpio_set_value(hook_spk_priv.sw_gpio, !EN_LEVEL);
		dev_info(dev, "Gpio [%s] set high!\n", prop_sw_gpio);	
	}
	return 0;
}
