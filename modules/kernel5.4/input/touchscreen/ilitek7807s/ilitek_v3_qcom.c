...
void ili_input_register(void)
{
	...
	input_set_capability(ilits->input, EV_KEY, KEY_AOD);
	...
	__set_bit(KEY_GESTURE_AOD, ilits->input->keybit);
...
static ssize_t ic_info_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%s\n", "ili7807s");
}
static DEVICE_ATTR_RO(ic_info);
