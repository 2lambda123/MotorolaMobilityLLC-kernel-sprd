...
int ili_get_tp_gesture_status(int items){
	ILI_ERR("lcd panel items %d\n",items);
	if( ilits == NULL )
		return 0;
	if( ilits->gesture == 0 )
		return 0;
...
EXPORT_SYMBOL(ili_get_tp_gesture_status);

static int ili_gesture_show(struct seq_file *sfile,void *v){
	if(ilits->gesture == 1)
		seq_printf(sfile,"1\n");
	else if(ilits->gesture == 2)
		seq_printf(sfile,"2\n");
	else if(ilits->gesture == 3)
		seq_printf(sfile,"3\n");
	else
		seq_printf(sfile,"0\n");
	return 0;
...
static ssize_t ili_gesture_store(struct   file *file,const char * buffer,size_t count,loff_t *pos)
{
	char dbg[10] = {0};
	int res = 0;
	uint8_t  state;
...
static int32_t ili_gesture_open(struct  inode *inode,struct file *file){
	return single_open(file,ili_gesture_show,NULL);
}


static struct file_operations ili_gesture_fops = {
	.owner = THIS_MODULE,
	.open = ili_gesture_open,
	.read = seq_read,
	.write = ili_gesture_store,
	.llseek = seq_lseek,
	.release = seq_release,
};
static struct proc_dir_entry *proc_dir_ilitek;
static struct proc_dir_entry *ILI_proc_gesture_entry;
...
void ili_node_init(void)
{
	...
	if(proc_android_touch_entry==NULL)
		ILI_ERR("create /proc/android_touch Failed!\n");
	ILI_proc_gesture_entry = proc_create(GESTURE_CONTROL,0644,proc_android_touch_entry,&ili_gesture_fops);

	if(ILI_proc_gesture_entry == NULL){
		ILI_ERR("create proc/android_touch/gesture_control Failed!\n");
	}else{
		ILI_ERR("create proc/android_touch/gesture_control Succeed!\n");
	}
