...
void ili_set_gesture_symbol(void)
{
	...
	cmd[2] = 0x0A;
	cmd[3] = 0x08;
	if(ilits->gesture == 1 )/*single tab only*/
	{	
		cmd[4] =0x00;
		cmd[5] =0x00;
		cmd[6] =0x08;
	}
	else if(ilits->gesture == 2)/*single tab +doule tab*/
	{
		cmd[4] =0x01;
		cmd[5] =0x00;
		cmd[6] =0x08;
	}
	else if(ilits->gesture == 3)/*double tab only*/
	{
		cmd[4] =0x01;
		cmd[5] =0x00;
		cmd[6] =0x00;
...
void ili_report_gesture_mode(u8 *buf, int len)
{
	...
	case GESTURE_SINGLECLICK:
		ILI_INFO("single Click key event\n");
             /***do what you want*****/
		input_report_key(input, KEY_GESTURE_AOD, 1);
		input_sync(input);
		input_report_key(input, KEY_GESTURE_AOD, 0);
		input_sync(input);
