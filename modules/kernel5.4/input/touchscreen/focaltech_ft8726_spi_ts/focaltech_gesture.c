...
static struct proc_dir_entry *fts_proc_gesture_entry;
...
int gesture_state =0;
int fts_get_tp_gesture_status(int items){
	printk("lcd panel items %d\n",items);
	if(gesture_state)
		return 1;
	return  0;
}
EXPORT_SYMBOL(fts_get_tp_gesture_status);


static int fts_gesture_control_show(struct seq_file *sfile,void *v){
	if(gesture_state == 1)
		seq_printf(sfile,"1\n");
        ...
        seq_printf(sfile,"3\n");
	else if(gesture_state == 0)
		seq_printf(sfile,"0\n");
	return 0;
}

static ssize_t fts_gesture_control_store(struct   file *file,const char * buffer,size_t count,loff_t *pos)
{
	char dbg[10] = {0};
	int res = 0;
	uint8_t  state;
...
static int32_t fts_gesture_control_open(struct  inode *inode,struct file *file){
	return single_open(file,fts_gesture_control_show,NULL);
}
static struct file_operations fts_gesture_control_fops = {
	.owner = THIS_MODULE,
	.open = fts_gesture_control_open,
	.read = seq_read,
	.write = fts_gesture_control_store,
	.llseek = seq_lseek,
	.release = seq_release,
};
...
*******************************************************/
int32_t fts_proc_init(void)
{

	proc_android_touch_entry = proc_mkdir("android_touch",NULL);
	if(proc_android_touch_entry==NULL)
		FTS_ERROR("create /proc/android_touch fail");
	fts_proc_gesture_entry = proc_create(GESTURE_CONTROL,0644,proc_android_touch_entry,&fts_gesture_control_fops);
	if(fts_proc_gesture_entry == NULL){
		FTS_ERROR("create proc/android_touch/gesture_control Failed!\n");
	}else{
		FTS_ERROR("create proc/android_touch/gesture_control Succeed!\n");
	}
...
static void fts_gesture_report(struct input_dev *input_dev, int gesture_id)
{
    ...
    printk(KERN_ERR"gesture_id:0x%x", gesture_id);
    switch (gesture_id) {
    case GESTURE_LEFT:
...
int fts_gesture_init(struct fts_ts_data *ts_data)
{
    ...
    input_set_capability(input_dev, EV_KEY, KEY_AOD);
