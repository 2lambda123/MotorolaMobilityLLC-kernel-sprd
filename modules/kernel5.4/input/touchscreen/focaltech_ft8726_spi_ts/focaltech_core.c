...
#define LCD_NAME "lcd_ft8726_csot_mipi_fhd"   //此处根据LCD命名修改
...
static struct workqueue_struct* fts_charger_notify_wq;
static struct work_struct	fts_update_charger;
static int fts_usb_state = 0;
static struct notifier_block fts_notify;
static int fts_charge_detection = 0 ;
int32_t fts_set_charger(uint8_t charger_on_off)
{
	int ret = 0;
	if (charger_on_off == USB_DETECT_IN) {
		ret = fts_write_reg(0x8B, 1);
		printk(KERN_ERR"fts set charger on cmd \n");
		FTS_INFO("fts set charger on cmd success");
	} else if (charger_on_off == USB_DETECT_OUT) {
		ret = fts_write_reg(0x8B, 0);
		printk(KERN_ERR"fts set charger off cmd \n");
		FTS_INFO("fts set charger off cmd success");
	} else {
		printk(KERN_ERR"Invalid charger parameter!\n");
		ret = -EINVAL;
	}
...
static void fts_update_charger_callback(struct work_struct *work)
{
	fts_set_charger(fts_charge_detection);
}

static int fts_notify_callback(struct notifier_block *self, unsigned long val,
		      void *dev)
{
	int ret = 0;
	...
	if (!psy) {
		printk(KERN_ERR"Couldn't get usbpsy\n");
		return -EINVAL;
	}
	if (!strcmp(psy->desc->name, "battery")) {
		if (psy && val == POWER_SUPPLY_PROP_STATUS) {
			...
			if (ret < 0) {
				 printk(KERN_ERR"Couldn't get POWER_SUPPLY_PROP_ONLINE rc=%d\n", ret);
				return ret;
			} else {
				if (fts_usb_state == 2)
                    fts_usb_state = prop.intval;
				if (fts_usb_state != prop.intval) {
                    printk(KERN_ERR"usb prop.intval =%d\n", prop.intval);
                    fts_usb_state = prop.intval;
                    fts_charge_detection = fts_usb_state;
                    if (fts_charger_notify_wq != NULL)
                        queue_work(fts_charger_notify_wq, &fts_update_charger);
				}
			}
		}
	}
...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
	...
	fts_notify.notifier_call = fts_notify_callback;
	ret = power_supply_reg_notifier(&fts_notify);
	if (ret < 0)
		printk(KERN_ERR"power_supply_reg_notifier failed\n");
	fts_charger_notify_wq = create_singlethread_workqueue("fts_charger_wq");
	INIT_WORK(&fts_update_charger, fts_update_charger_callback);
	if(!fts_charger_notify_wq) {
		printk(KERN_ERR"allocate nvt_charger_notify_wq failed\n");
	}
	psy = power_supply_get_by_name("battery");
	if (!psy) {
		printk(KERN_ERR"Couldn't get usbpsy\n");
			...
			if (psy) {
				...
				if (ret < 0) {
					...
					if (fts_usb_state != prop.intval) {
						printk(KERN_ERR"usb prop.intval =%d\n", prop.intval);
						fts_usb_state = prop.intval;
						fts_charge_detection  = prop.intval;
						if (fts_charger_notify_wq != NULL)
							queue_work(fts_charger_notify_wq, &fts_update_charger);
					}
				}
...
int fts_ts_resume(struct device *dev)
{
    ...
    fts_wait_tp_to_valid();
    fts_ex_mode_recovery(ts_data);
	if(fts_charge_detection == 0x01)
	{
		fts_write_reg(FTS_REG_CHARGER_MODE_EN, 0x01);
	}
	else
		fts_write_reg(FTS_REG_CHARGER_MODE_EN, 0x00);
