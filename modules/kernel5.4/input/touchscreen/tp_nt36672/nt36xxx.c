...
static struct workqueue_struct* nvt_charger_notify_wq;
static struct work_struct	nvt_update_charger;
static int nvt_usb_state = 0;
static struct notifier_block nvt_notify;
static int nvt_charge_detection = 0 ;
...
static void nvt_update_charger_callback(struct work_struct *work)
{
	nvt_set_charger(nvt_charge_detection);
}

static int nvt_notify_callback(struct notifier_block *self, unsigned long val,
		      void *dev)
{
	int ret = 0;
	...
	if (!strcmp(psy->desc->name, "battery")) {
		if (psy && val == POWER_SUPPLY_PROP_STATUS) {
			...
			if (ret < 0) {
				...
				if (nvt_usb_state == 2)
                                        nvt_usb_state = prop.intval;
                                if (nvt_usb_state != prop.intval) {
                                        NVT_LOG("usb prop.intval =%d\n", prop.intval);
                                        nvt_usb_state = prop.intval;
					nvt_charge_detection = nvt_usb_state;
                                        if (nvt_charger_notify_wq != NULL)
                                                queue_work(nvt_charger_notify_wq, &nvt_update_charger);
				}
			}
		}
	}
...
void nvt_pinctrl_init(struct device* dev)
{
	...
	if (IS_ERR_OR_NULL(ts->pins_extconf1)) {
		NVT_ERR("Pin state[extconf1] not found");
	ret = PTR_ERR(ts->pins_extconf1);
	// goto err_pinctrl_lookup;
	}
}

int nvt_pinctrl_select_irq_eic(struct nvt_ts_data  *ts)
{
	int ret = 0;
...
int nvt_pinctrl_select_irq_gpio(struct nvt_ts_data *ts)
{
	int ret = 0;
...
void nvt_eic_irq_disable(void)
{
	unsigned long irqflags;
	//FTS_FUNC_ENTER();
	spin_lock_irqsave(&ts->irq_lock, irqflags);
	if (!ts->irq_disabled) {
		disable_irq_nosync(ts->irq_eic);
		ts->irq_disabled = true;
	NVT_ERR("irq disable %d\n",ts->irq_disabled);
	}
	spin_unlock_irqrestore(&ts->irq_lock, irqflags);
	//FTS_FUNC_EXIT();
}
void nvt_eic_irq_enable(void)
{
	unsigned long irqflags = 0;
	//FTS_FUNC_ENTER();
	spin_lock_irqsave(&ts->irq_lock, irqflags);
	if (ts->irq_disabled) {
		enable_irq(ts->irq_eic);
...
int nvt_get_tp_gesture_status(int items){
	NVT_ERR("lcd panel items %d\n",items);
	if( gesture_state )
		return 1;
	return  0;
}
EXPORT_SYMBOL(nvt_get_tp_gesture_status);

void nvt_esd_check_recovery(void)
{
		mutex_lock(&ts->lock);
		NVT_LOG("do ESD recovery ");
		/* do esd recovery, reload fw */
		nvt_update_firmware(BOOT_UPDATE_FIRMWARE_NAME);
		mutex_unlock(&ts->lock);
}
EXPORT_SYMBOL(nvt_esd_check_recovery);
...
static struct file_operations nvt_gesture_fops = {
	.owner = THIS_MODULE,
...
static int32_t nvt_flash_proc_init(void)
{
	...
	if(NVT_proc_gesture_entry == NULL){
		...
		NVT_ERR("create proc/android_touch/gesture_control Succeed!\n");
	}
	NVT_LOG("============================================================\n");
...
static ssize_t ic_info_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%s\n", "nt36672c");
}

static DEVICE_ATTR(ic_info, 0664, ic_info_show, NULL);
...
static int32_t nvt_parse_dt(struct device *dev)
{
	...
	ts->irq_use_eic = of_get_named_gpio_flags(np, "touch,eic-irq-gpio", 0, &ts->irq_use_eic_flags);
	if(ts->irq_use_eic < 0)
		NVT_ERR("Unable to get eic_irq_gpio\n");
...
static int nvt_gpio_config(struct nvt_ts_data *ts)
{
	...
	if (!gpio_is_valid(ts->irq_use_eic)) {
		NVT_ERR("Invalid  eic gpio \n");
		return -EBADR;
	}
	ret = gpio_request(ts->irq_use_eic, "TP_EIC");
	if (ret) {
		NVT_ERR("Request EIC GPIO failed, ret = %d\n", ret);
		gpio_free(ts->irq_use_eic);
		ret = gpio_request(ts->irq_use_eic, "TP_EIC");
		if (ret < 0) {
			NVT_ERR("Retrying request RESET GPIO still failed , ret = %d\n", ret);
		}
	}else{
		ret = gpio_direction_input(ts->irq_use_eic);
		if (ret) {
			NVT_ERR("[GPIO]set_direction for eic irq gpio failed\n");
		}
	}
...
#endif
#if defined(CONFIG_PM) && NVT_PATCH_COMERR_PM

    if ((ts->suspended) && (ts->pm_suspend)) {
		NVT_LOG("wait_for_completion_timeout....");
        ret = wait_for_completion_timeout(
                  &ts->pm_completion,
                  msecs_to_jiffies(NVT_TIMEOUT_COMERR_PM));
        if (!ret) {
            NVT_ERR("Bus don't resume from pm(deep),timeout,skip irq");
            return IRQ_HANDLED;
        }
    }
#endif
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
	...
	ts->irq_eic = gpio_to_irq(ts->irq_use_eic);
	ts->irq_use_eic_flags = IRQF_TRIGGER_FALLING | IRQF_ONESHOT | IRQF_NO_SUSPEND;
	NVT_ERR("irq_eic:%d, eic_flag:%x", ts->irq_eic, ts->irq_use_eic_flags);
	ret = request_threaded_irq(ts->irq_eic,NULL,nvt_ts_work_func,ts->irq_use_eic_flags,"nvteic",ts);
	NVT_ERR("irq handler register %d\n", ret);
	nvt_eic_irq_disable();
	...
	/*
	 * support SPRD SC9863A
	 * /sys/devices/virtual/touchscreen/light_suspend/ts_suspend
	 */
...
#endif
#endif //CONFIG_ARCH_SPRD
	nvt_notify.notifier_call = nvt_notify_callback;
	ret = power_supply_reg_notifier(&nvt_notify);
	if (ret < 0)
		NVT_ERR("power_supply_reg_notifier failed\n");
	nvt_charger_notify_wq = create_singlethread_workqueue("nvt_charger_wq");
	INIT_WORK(&nvt_update_charger, nvt_update_charger_callback);
	if(!nvt_charger_notify_wq) {
			...
			if (psy) {
				...
				if (ret < 0) {
					...
					if (nvt_usb_state != prop.intval) {
						NVT_LOG("usb prop.intval =%d\n", prop.intval);
						nvt_usb_state = prop.intval;
						nvt_charge_detection  = prop.intval;
						if (nvt_charger_notify_wq != NULL)
							queue_work(nvt_charger_notify_wq, &nvt_update_charger);
					}
				}
...
static int nvt_gesture_set_mode(unsigned int mode)
{

	NVT_LOG("Gesture mode is %d!",mode);
	switch(mode+1)
	{
		case 1:
			nvt_cmd_ext_store(0x7B,0x01); //mode 0,IDLE
			break;
		case 2:
			nvt_cmd_ext_store(0x7B,0x02); //mode 1,singletap only,default
			break;
		case 3:
			nvt_cmd_ext_store(0x7B,0x03); //mode 2,both single tap & double tap
			break;
		case 4:
			nvt_cmd_ext_store(0x7B,0x04); //mode 3,double tap only
			break;
		default:
...
int32_t nvt_ts_suspend(struct device *dev)
{
	...
	if(gesture_state) {
	    ...
	    nvt_gesture_set_mode(gesture_state);
		buf[0] = EVENT_MAP_HOST_CMD;
		buf[1] = 0x13;
		...
		//nvt_irq_enable(false);
		nvt_pinctrl_select_irq_eic(ts);
		enable_irq_wake(ts->irq_eic);
...
int32_t nvt_ts_resume(struct device *dev)
{
	...
	nvt_irq_enable(true);
	nvt_eic_irq_disable()
	nvt_pinctrl_select_irq_gpio(ts);
#else
	nvt_irq_enable(true);
	nvt_eic_irq_disable();
	nvt_pinctrl_select_irq_gpio(ts);
...
static int nvt_pm_resume(struct device *dev)
{
    ...
    NVT_LOG("system resumes from pm_suspend");
