 ...
 *
 * Author: Alec <like@awinic.com.cn>
 *
...
#include "leds-aw99703.h"

#define AW99703_LED_DEV "aw99703-bl"
#define AW99703_NAME "sprd-backlight"
...
/******************************************************
 *
 * sys group attribute: boost_en
 *
 ******************************************************/
static ssize_t aw99703_boost_en_store(struct device *dev,
			struct device_attribute *attr, const char *buf, size_t count)
{
	struct aw99703_data *aw99703 = dev_get_drvdata(dev);
	int i;
	...
	if (buf[0] == 49) {
		/*enter boost mode , The current is boosted from 20mA to 24.8ma*/
		for (i = 0x13; i <= 0x18; i++) {
			aw99703_i2c_write_bit(aw99703->client,
						AW99703_REG_LEDCUR,
						AW99703_LEDCUR_BLFS_MASK,
						(i + 1) << 3);
			mdelay(200);
		...
		/*exit boost mode , The current is boosted from 24.8ma to 20mA*/
		for (i = 0x19; i >= 0x14; i--) {
			aw99703_i2c_write_bit(aw99703->client,
								AW99703_REG_LEDCUR,
								AW99703_LEDCUR_BLFS_MASK,
...
static ssize_t aw99703_boost_en_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	struct aw99703_data *aw99703 = dev_get_drvdata(dev);
	unsigned char reg_val = 0;
	...
	aw99703_i2c_read(aw99703->client, AW99703_REG_LEDCUR, &reg_val);
	len += snprintf(buf, PAGE_SIZE - len, "reg:0x03=0x%02x\n", reg_val);

	return len;
}

static DEVICE_ATTR(reg, 0664, aw99703_i2c_reg_show, aw99703_i2c_reg_store);
static DEVICE_ATTR(boost_en, 0664, aw99703_boost_en_show,
								   aw99703_boost_en_store);

static struct attribute *aw99703_attributes[] = {
	&dev_attr_reg.attr,
	&dev_attr_boost_en.attr,
	NULL
};
...
static int aw99703_remove(struct i2c_client *client)
{
	...
	sysfs_remove_group(&client->dev.kobj, &aw99703_attribute_group);
	kfree(drvdata);
	return 0;
}
