...
// QCOM and MTK54 use standard GPIO definition
    ...
    if (!np) {
        ...
        st21nfc_dev->sc27xx_refout = kzalloc(sizeof(struct pmic_refout), GFP_KERNEL);
        if (!st21nfc_dev->sc27xx_refout) {
            pr_err("%s- can't alloc memory for pmic refout", __func__);
            return -ENOMEM;
        }

        ret = of_property_read_u32_index(np, "regsw", 0, &st21nfc_dev->sc27xx_refout->regsw);
        if (ret) {
            pr_err("Get base register failed\n");
            return -EINVAL;
        }

        ret = of_property_read_u32_index(np, "refnum", 0, &st21nfc_dev->sc27xx_refout->refnum);
        if (ret) {
        ...
        of_node_put(np);
        st21nfc_dev->sc27xx_refout->regmap = dev_get_regmap(pmic_device->dev.parent, NULL);
        if (!st21nfc_dev->sc27xx_refout->regmap) {
            put_device(&pmic_device->dev);
            pr_err("%s- cant't get pmic regmap device\n", __func__);
            return -ENODEV;
        }
        put_device(&pmic_device->dev);
        reg_val = regmap_read(st21nfc_dev->sc27xx_refout->regmap, st21nfc_dev->sc27xx_refout->regsw, &reg_val);
