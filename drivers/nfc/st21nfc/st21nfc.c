...
/*
static int nfc_clk_select(struct st21nfc_device *nfc_dev)
{
			 ...
			 const struct i2c_device_id *id)
{
	...
	struct st21nfc_device *st21nfc_dev;  
	struct device *dev = &client->dev;
   // pr_err("wangzemin 11%s : 1111 stat  st21nfc_probe\n", __func__);
   
       //st21
    //nfc_clk_select(st21nfc_dev);
	 pr_err(" st21nfc before open refout4 1026_4");
	 pmic_refout_update(4, 1);
	 pr_err("st21nfc after open refout4 1026_4");
   
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		pr_err("%s : st21nfc need I2C_FUNC_I2C\n", __func__);
		return -ENODEV;
	}
...
// QCOM and MTK54 use standard GPIO definition
	ret = acpi_dev_add_driver_gpios(ACPI_COMPANION(dev),
					acpi_st21nfc_gpios);
	if (ret)
		pr_debug("st21nfc Unable to add GPIO mapping table\n");

// QCOM and MTK54 use standard GPIO definition
	st21nfc_dev->gpiod_irq = devm_gpiod_get(dev, "irq", GPIOD_IN);
	if (IS_ERR_OR_NULL(st21nfc_dev->gpiod_irq)) {
		pr_err("%s :st21nfc  Unable to request irq-gpios\n", __func__);
		return -ENODEV;
	}
...
// QCOM and MTK54 use standard GPIO definition
	...
	if (IS_ERR_OR_NULL(st21nfc_dev->gpiod_reset)) {
		pr_err("%s :st21nfc  Unable to request reset-gpios\n", __func__);
		return -ENODEV;
	}
...
// QCOM and MTK54 use standard GPIO definition
	...
	if (IS_ERR_OR_NULL(st21nfc_dev->gpiod_pidle)) {
		pr_err("[OPTIONAL] %s: st21nfc Unable to request pidle-gpio\n",
			__func__);
		ret = 0;
	} else {
		if (!device_property_read_bool(dev, "st,pidle_active_low")) {
			pr_err("%s:st21nfc [OPTIONAL] pidle_active_low not set\n", __func__);
			st21nfc_dev->pidle_active_low = false;
		} else {
			pr_err("%s:st21nfc [OPTIONAL] pidle_active_low set\n", __func__);
		...
		ret = irq_set_irq_type(st21nfc_dev->irq_pw_stats_idle,
				       IRQ_TYPE_EDGE_BOTH);
		if (ret) {
			pr_err("%s :st21nfc  set_irq_type failed\n", __func__);
  ...
  //pr_err("[OPTIONAL] %s : Unable to request clkreq-gpios\n", __func__);
	...
	if (IS_ERR_OR_NULL(st21nfc_dev->gpiod_clkreq)) {
		pr_warn("st21nfc [OPTIONAL] %s : Unable to request clkreq-gpios\n",
			__func__);
		ret = 0;
	} else {
		if (!device_property_read_bool(dev, "st,clk_pinctrl")) {
			pr_debug("%s:st21nfc [OPTIONAL] clk_pinctrl not set\n",
				 __func__);
			st21nfc_dev->pinctrl_en = 0;
		} else {
			pr_debug("%s:st21nfc [OPTIONAL] clk_pinctrl set\n",
				 __func__);
		...
		if (ret < 0) {
			pr_err("%s :st21nfc st21nfc_clock_select failed\n", __func__);
	...
	if (of_property_read_u32(dev->of_node, "i2c-retry",
		...
		pr_debug("%s :st21nfc  i2c-retry = %d\n", __func__,
			 client->adapter->retries);
	}

#ifdef WITH_PING_DURING_PROBE
	if ((ret = st21nfc_ping(st21nfc_dev))) {
		pr_err("%s: st21nfc Did not get CORE_RESET_NTF, hardware issue? (%d)\n", __func__, ret);
		return ret;
	}
#endif // WITH_PING_DURING_PROBE
   ...
   // pr_err("%s : st21nfc 444misc_register st21nfc_probe\n", __func__);
	...
	//pr_err("%s : 444misc_register st21nfc_probe\n", __func__);
	if (ret) {
		pr_err("%s : st21nfc misc_register failed\n", __func__);
		goto err_misc_register;
	}

	ret = sysfs_create_group(&dev->kobj, &st21nfc_attr_grp);
	if (ret) {
		pr_err("%s : st21nfc sysfs_create_group failed\n", __func__);
    ...
    pr_err("wangzemin st21nfc  OK ok  %s : st21nfc_probe OK OK\n", __func__);
...
{
	pr_err("wangzemin %s : st21nfc_dev_init\n", __func__);
	printk("wangzemin printk  %s : set_irq_type failed\n", __func__);
